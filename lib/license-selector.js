(function() {
  ...
  History = (function() {
    function History(parent, licenseSelector) {
      ...
      this.current = 0;
      this.historyStack = [];
      this.prevButton = $('<button/>').addClass('ls-history-prev').click(function() {
        return _this.go(_this.current - 1);
      });
      this.nextButton = $('<button/>').addClass('ls-history-next').click(function() {
        return _this.go(_this.current + 1);
      });
      this.restartButton = $('<button/>').addClass('ls-restart').append($('<span/>').addClass('icon-ccw')).append(' Start again').click(function() {
        return _this.licenseSelector.restart();
      });
      this.progress = $('<div/>').addClass('ls-history-progress');
      $('<div/>').addClass('ls-history').append(this.restartButton).appendTo(this.parent);
      this.update();
    }
    ...
    History.prototype.pushState = function(state) {
      ...
      if (this.historyStack.length > this.current + 1) {
        this.historyStack = this.history.slice(0, this.current);
      }
      ...
      if (progressBarBlocks > index) {
        this.progress.children(":gt(" + this.index + ")").remove();
      } else {
        that = this;
    ...
    History.prototype.popState = function() {
      var state;
      state = this.historyStack.pop();
      this.current -= 1;
      this.progress.children(':last').remove();
      this.update();
      return state;
    };
  ...
  Search = (function() {
    function Search(parent, licenseList) {
      ...
      container = $('<div/>').addClass('ls-search').append(this.textbox).appendTo(this.parent);
    }
